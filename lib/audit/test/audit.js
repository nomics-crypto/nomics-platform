...
module.exports = {
  ...
  markets: async () => {
    ...
    try {
      ...
      data.forEach((m) => au.assertPropertyInSet(m, 'base_type', ['crypto', 'fiat']));
      data.forEach((m) => au.assertStringProperty(m, 'quote'));
      data.forEach((m) => au.assertPropertyInSet(m, 'quote_type', ['crypto', 'fiat']));
      data.forEach((m) => au.assertBooleanProperty(m, 'active'));

      // optional
      //
      data.forEach((m) => au.assertStringProperty(m, 'settlement', { required: false }));
      data.forEach((m) => au.assertPropertyInSet(m, 'settlement_type', ['crypto', 'fiat'], { required: false }));
      data.forEach((m) => au.assertStringProperty(m, 'quote', { required: false }));
      data.forEach((m) => au.assertPropertyInSet(m, 'quote_type', ['crypto', 'fiat'], { required: false }));
      data.forEach((m) => {
  ...
  tradesByTimestamp: async () => {
    ...
    for (let i = 0; i < markets.length && !success; i++) {
      ...
      try {
        ...
        trades.forEach((t) => au.assertStringProperty(t, 'order', { required: false }));
        trades.forEach((t) =>
          au.assertPropertyInSet(t, 'type', ['market', 'limit', 'ask', 'bid'], { required: false }),
        );
        trades.forEach((t) => au.assertPropertyInSet(t, 'side', ['buy', 'sell'], { required: false }));
